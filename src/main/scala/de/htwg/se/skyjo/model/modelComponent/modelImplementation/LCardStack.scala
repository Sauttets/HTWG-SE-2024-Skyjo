...
case class LCardStack(stack: List[CardInterface], trashstack: List[CardInterface]) extends CardStackStrategy:
    def this(stackList:List[CardInterface])  =   this(stackList,List(CardBuilder().opened(true).build()))
    ...
    override def discard(card: CardInterface): LCardStack = copy(stack, trashstack:+card.open())
